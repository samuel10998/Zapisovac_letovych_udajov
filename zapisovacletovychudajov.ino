...
bool saveLocation() {
    ...
    if (dataFile) { // Kontrola, či sa súbor úspešne otvoril
        ...
        float correctedPressure = bmp280.readPressure() / 100.0 + 21; // +21 čo je offset po kalibrácií
        float altitude = bmp280.readAltitude(correctedPressure); // Nadmorská výška hodnota
        ...
        dataFile.write('\t');
        dataFile.write('\t');
        dataFile.print(correctedPressure, 2);  // Tlak s 2 desatinnými miestami
        dataFile.write('\t');
        dataFile.write('\t');
        dataFile.print(altitude); // Nadmorská výška
        dataFile.write('\t');
        dataFile.write('\t');
